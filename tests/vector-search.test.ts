...
describe('quantizeValue', () => {
  it('maps sentinel, zero, half and one to stable Uint8 buckets', () => {
    expect(quantizeValue(-1)).toBe(0);
    expect(quantizeValue(0)).toBe(128);
    expect(quantizeValue(0.5)).toBe(192);
    expect(quantizeValue(1)).toBe(255);
    expect(quantizeValue(2)).toBe(255);
  });
});
...
describe('quantizeVector', () => {
  it('writes into the provided output buffer', async () => {
    ...
    expect(Array.from(output)).toEqual([0, 128, 160, 255]);
  });
});
...
describe('topKSearch', () => {
  it('returns nearest vectors sorted by ascending squared distance', async () => {
    ...
    const neighbors = await topKSearch(index, new Uint8Array([12, 13]), 3, result);
  ...
  it('resets result buffers between calls', async () => {
    const index = createVectorSearchIndexFrom(
      1,
      new Uint8Array([1, 10, 100]),
      new Uint8Array([0, 1, 0]),
    );
    const result = createTopKResult(2);

    await topKSearch(index, new Uint8Array([100]), 2, result);
    expect(Array.from(result.indexes)).toEqual([2, 1]);

    await topKSearch(index, new Uint8Array([1]), 2, result);
  ...
  it('handles topK larger than result capacity and empty indexes', async () => {
    const empty = createVectorSearchIndexFrom(2, new Uint8Array([]), new Uint8Array([]));
    const result = createTopKResult(1);

    await topKSearch(empty, new Uint8Array([1, 2]), 5, result);
  ...
  it('throws when vector and label sizes are inconsistent', () => {
    expect(() =>
      createVectorSearchIndexFrom(2, new Uint8Array([1, 2, 3]), new Uint8Array([1])),
    ).toThrow('Vector data length must be divisible by dimensions');

    expect(() =>
      createVectorSearchIndexFrom(2, new Uint8Array([1, 2, 3, 4]), new Uint8Array([1])),
